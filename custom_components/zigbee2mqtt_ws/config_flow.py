...
async def _test_connection(
    ...
    # Z2M frontend uses token as query param, not Authorization header
    params = {}
    if auth_token:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.ws_connect(url, params=params, ssl=False, timeout=aiohttp.ClientTimeout(total=10)) as ws:
