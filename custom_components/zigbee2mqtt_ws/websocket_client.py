...
class Z2MWebSocketClient:
    ...
    async def _handle_raw(self, raw: str) -> None:
        ...
        # Z2M sometimes sends payload as a JSON string instead of parsed value.
        # Normalise it so callers always get Python objects.
        if isinstance(payload, str):
            try:
                payload = json.loads(payload)
            except (json.JSONDecodeError, TypeError):
                pass  # keep as plain string (e.g. "online" / "offline")
    ...
    async def set_state(self, friendly_name: str, payload: dict) -> None:
        await self.publish(f"{self._base_topic}/{friendly_name}/set", payload)

    async def get_state(self, friendly_name: str, payload: dict) -> None:
        await self.publish(f"{self._base_topic}/{friendly_name}/get", payload)

    async def bridge_request(self, endpoint: str, payload: Any) -> None:
        await self.publish(f"{self._base_topic}/bridge/request/{endpoint}", payload)
