...
class Z2MEntity(Entity):
    ...
    async def _publish_set(self, payload: dict) -> None:
        ...
        if "state" in payload:
            current_state = coordinator.device_states.get(friendly_name, {}).get("state", "UNKNOWN")
            new_state = "ON" if payload["state"] == "ON" else "OFF"
            coordinator._pending_state[friendly_name] = (new_state, str(current_state).upper(), time.monotonic())
