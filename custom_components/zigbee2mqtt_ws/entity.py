...
class Z2MEntity(Entity):
    ...
    async def _publish_set(self, payload: dict) -> None:
        ...
        # Track pending state change with timestamp
        if "state" in payload:
            state_value = "ON" if payload["state"] == "ON" else "OFF"
            coordinator._pending_state[friendly_name] = (state_value, time.monotonic())
