...
class Z2MCoordinator:
    ...
    def _on_message(self, topic: str, payload: Any) -> None:
        ...
        elif topic.startswith(f"{base}/") and topic.endswith("/availability"):
            # zigbee2mqtt/some/friendly/name/availability
            friendly_name = topic[len(base) + 1 : -len("/availability")]
            self._handle_availability(friendly_name, payload)

        elif (
            topic.startswith(f"{base}/")
            and not topic.startswith(f"{base}/bridge/")
        ):
