...
class Z2MCoordinator:
    ...
    def __init__(
        ...
        # Track pending state changes to ignore immediate echo from Z2M
    ...
    def _handle_device_state(self, friendly_name: str, payload: Any) -> None:
        ...
        # Check if this is a state change we initiated
        pending = self._pending_state.pop(friendly_name, None)
        if pending and "state" in payload:
            received = str(payload["state"]).upper()
            if received == pending:
                _LOGGER.debug("Ignoring echo for %s: %s", friendly_name, received)
                # Still update internal state but skip dispatcher to prevent toggle
