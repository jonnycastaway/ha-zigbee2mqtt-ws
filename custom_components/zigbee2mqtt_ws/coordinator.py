...
class Z2MCoordinator:
    ...
    def __init__(
        ...
        # Track pending state changes with previous state
        # friendly_name -> (expected_state, previous_state, timestamp)
        self._pending_state: dict[str, tuple[str, str, float]] = {}
    ...
    def _handle_device_state(self, friendly_name: str, payload: Any) -> None:
        ...
        # Always update internal state first (so is_on returns correct value)
        ...
        if pending is not None:
            expected_state, previous_state, timestamp = pending
            
            if now - timestamp <= 2.0 and "state" in payload:
                received = str(payload["state"]).upper()
                _LOGGER.debug("State check for %s: got=%s expected=%s previous=%s", 
                           friendly_name, received, expected_state, previous_state)
                
                if received == expected_state:
                    _LOGGER.debug("Ignoring echo for %s - skipping dispatch", friendly_name)
                    self._pending_state.pop(friendly_name, None)
                    should_dispatch = False
                elif received == previous_state:
                    _LOGGER.debug("Skipping OLD state for %s - skipping dispatch", friendly_name)
                    should_dispatch = False
                else:
                    _LOGGER.debug("External state change for %s - dispatching", friendly_name)
                    self._pending_state.pop(friendly_name, None)
