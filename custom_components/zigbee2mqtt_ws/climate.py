...
class Z2MClimate(Z2MEntity, ClimateEntity):
    ...
    def __init__(self, coordinator: Z2MCoordinator, device: dict) -> None:
        ...
        if "system_mode" in feature_props:
            modes_raw = feature_props["system_mode"].get("values", [])
        ...
        if "occupied_heating_setpoint" in feature_props:
            self._setpoint_prop = "occupied_heating_setpoint"
        else:
            self._setpoint_prop = "current_heating_setpoint"
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        # Reverse map
        reverse = {v: k for k, v in Z2M_HVAC_MAP.items()}
        z2m_mode = reverse.get(hvac_mode, str(hvac_mode))
