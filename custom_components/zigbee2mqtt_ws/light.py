...
async def async_setup_entry(
    ...
    def _add_devices(devices: list[dict]) -> None:
        _LOGGER.debug("Light platform: received SIGNAL_DEVICES_UPDATED with %d devices", len(devices))
        new_entities = []
        for device in devices:
            ...
            _LOGGER.debug("Light platform: checking device %s, definition=%s", ieee, definition is not None)
            if _is_light(definition):
                known.add(ieee)
                new_entities.append(Z2MLight(coordinator, device))
        if new_entities:
            _LOGGER.info("Light platform: adding %d new light entities", len(new_entities))
