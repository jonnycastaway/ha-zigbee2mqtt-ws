...
# Properties that are binary sensors, not sensors
...
def _get_sensor_exposes(definition: dict | None) -> list[dict]:
    """Return numeric/enum exposes suitable for sensors."""
    if not definition:
        return []
    result = []
    for expose in definition.get("exposes", []):
        ...
        # Skip binary sensor props (handled by binary_sensor platform)
        if prop in BINARY_PROPS:
            continue

        if t in ("numeric", "enum", "text") and prop:
            # Read-only or read-write – include all readable ones
            if expose.get("access", 1) & 1:
                result.append(expose)

        # Check composite features
        if t == "composite" or t == "climate":
            for feat in expose.get("features", []):
                ...
                if ft in ("numeric", "enum") and fp and feat.get("access", 1) & 1:
                    # Only include pure read-only or already-handled ones
                    if not (feat.get("access", 0) & 2):
                        result.append(feat)
...
async def async_setup_entry(
    ...
    def _add_devices(devices: list[dict]) -> None:
        ...
        for device in devices:
            ...
            for expose in _get_sensor_exposes(definition):
                prop = expose.get("property", "")
                uid = f"{ieee}_{prop}"
                if uid in known:
                    continue
...
class Z2MSensor(Z2MEntity, SensorEntity):
    ...
    def native_value(self):
        return self._get_state_value(self._prop)
